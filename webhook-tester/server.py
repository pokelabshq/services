...
class Handler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        elif p == "/api/requests": self.send_json(get_requests())
    ...
    def page(self):
        ...
        h.append("<style>body{font-family:system-ui,monospace;max-width:800px;margin:40px auto;padding:0 20px;background:#0a0a0a;color:#e0e0e0}h1{color:#a78bfa}button{background:#a78bfa;color:#000;border:none;padding:8px 16px;border-radius:4px;cursor:pointer;font-weight:700}.card{background:#1a1a2e;border:1px solid #333;border-radius:8px;padding:14px;margin:10px 0}.req{background:#111;border-radius:6px;padding:10px;margin:6px 0;font-size:.85em;word-break:break-all}.m{color:#22c55e;font-weight:700}.t{color:#666;font-size:.8em}</style></head><body>")
        ...
        h.append("<script>")
        h.append("function n(){fetch('/api/new').then(r=>r.json()).then(d=>{document.getElementById('u').textContent=d.url})}")
        h.append("function l(){fetch('/api/requests').then(r=>r.json()).then(d=>{")
